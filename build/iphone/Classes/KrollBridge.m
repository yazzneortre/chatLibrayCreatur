...
NSString * chatLibrayCreatur$ModuleRequireFormat = @"(function(exports){"
...
@implementation chatLibrayCreaturObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_chatlibraycreatur);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create chatLibrayCreatur global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "chatLibrayCreatur" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_chatlibraycreatur = [[chatLibrayCreaturObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_chatlibraycreatur];
	
	NSString *_chatlibraycreaturNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _chatlibraycreaturNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_chatlibraycreatur valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:chatLibrayCreatur$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.chatlibraycreatur.kroll" 
