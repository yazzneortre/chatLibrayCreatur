...
//Rare exceptions to indicate a bug in the _chatlibraycreatur code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_chatlibraycreatur = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_chatlibraycreatur]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.chatlibraycreatur.module"
